...
| `system` | 148 | Subtypes: `turn_duration` (125), `away_summary` (21), `api_error` (2). First-consumer smoke tests on 2.1.197 additionally observed `model_refusal_fallback` (see error shapes, below). From 2.1.205, `turn_duration` records can carry `pendingBackgroundAgentCount`. From 2.1.267, sessions with a Stop hook configured write `stop_hook_summary` (per-turn hook run summary; keys in the source note above) |
| `attachment` | 177 | Harness-injected context: `task_reminder`, `skill_listing`, `deferred_tools_delta`, `queued_command`, `agent_listing_delta`, `date_change`, `diagnostics`, `command_permissions`, `edited_text_file`. A drift probe on 2.1.197 additionally observed the envelope key `attachment.needsAuthMcpServers` in headless runs; additive, absorbed into the vocabulary baseline. Later versions add `hook_success` (2.1.267; per-hook `command`, `durationMs`, `exitCode`, `stdout`, `stderr`), `remote_session_change` (`managedCommit`/`managedPr` bools, 2.1.274), `batching_reminder_sent` (`clearAt`, 2.1.274), and `prompt_snapshot` (`reason`), among others; every type becomes an `attachment/<type>` system event. |
...
| Tool | sidecar keys |
| --- | --- |
| Bash | `stdout`, `stderr`, `interrupted`, `isImage`, `noOutputExpected`, `backgroundTaskId`, `persistedOutputPath`, `dangerouslyDisableSandbox` (bool; 2.1.205), `bashEditDiff` (2.1.274; the harness's own diff of files a shell command changed: `files[{filePath, hunks[]}]`, `moreFiles`, `changedFiles`, `shared`; the only structured record of Bash-mediated edits, so a promotion candidate, never applied by default), `gitOperation` (2.1.212; structured git metadata on commit/push commands: `commit{sha, kind}`, `push{branch}`), `backgroundCwdHint` (str; 2.1.212, background tasks) |
...
## Probe round of 2026-09-25 (2.1.274): failures, images, MCP, notebooks, custom agents, peer messaging

Triggered by the Copilot adapter work (see `copilot-format-inventory.md`), which showed how much the happy-path probes leave unobserved. Six headless probes plus a review of the interactive corpus on the same release; no adapter change was needed, the baseline was regenerated, and `testdata/tools.jsonl` pins the shapes.

- **Failure shapes.** A Bash command that exits nonzero is `is_error: true` with content `Exit code 1\n<stderr>` and the sidecar a plain error string. A `Read` of an absent path and an `Edit` whose `old_string` is missing are `is_error: true` with a `<tool_use_error>` wrapper in the content. A `WebFetch` that 404s is **not** `is_error`: the content is prose ("The server returned HTTP 404 Not Found...") and the sidecar carries `code: 404`, `codeText`, `bytes: 0`, so the adapter's `fetch` promotion (status from the sidecar) is the only structured signal. An unknown skill (`Skill {skill, args}`) is `is_error: true` ("Unknown skill: ..."); project skills live under `.claude/skills/`, not `.agents/skills/`.
- **Images.** `Read` of a PNG returns a `tool_result` whose content is `[{type: "image", source: {type: "base64", data, media_type}}]` with the sidecar `{type: "image", file: {base64, type, originalSize}}`: the bytes are inlined twice. The adapter maps the block to an `image` content block verbatim.
- **MCP tools.** A call is an ordinary `tool_use` named `mcp__<server>__<tool>` (probed with `--mcp-config` and a stdio server), its result an array of text blocks, and the `assistant` record carries `attributionMcpServer`/`attributionMcpTool`. Before this round the corpus held MCP names only inside `attachment` tool listings, never as a call.
- **NotebookEdit** `{notebook_path, cell_id, new_source}` returns a one-line confirmation with a sidecar of `new_source`, `old_source`, `cell_type`, `language`, `edit_mode`, `cell_id`, `error` (null), `notebook_path`.
- **Custom agents** (`--agents` JSON) run as `Agent {subagent_type: "<name>", ...}`; the result's second text block carries the `agentId` and a `<usage>` line, and the sidecar adds `agentType`, `totalDurationMs`, `totalTokens`, `totalToolUseCount`, `toolStats`, `usage`, and `harnessNoteCount`/`harnessTailCount`/`harnessSectionHash`. The subagent file under `subagents/` is the same layout as before.
- **Peer messaging and hand-back** (interactive corpus, 2.1.274). `SendMessage {to, summary, message, type, recipient, content}` targets another session or agent; its result is a JSON text with a `pin {id, name, ref}` (sidecar `success`, `message`, `pin`). The reply arrives as a `user` record with `isMeta: true` and `origin {kind: "peer", from, senderTaskId, body, handback: true}`, content "Another Claude session sent a message:\n<agent-message from=...>": the adapter already classifies it as harness-origin via `isMeta`. A subagent ends its work with a `SubagentHandback {message}` tool call in its own file. `attachment` records gained `attachment.isMeta`. All additive; the previous baseline (generated from the same release) lacked them because they only arise in interactive multi-session use.
