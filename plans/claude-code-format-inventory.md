...
| type | n | content |
| --- | --- | --- |
| `cost-state` | 3 (of 3 interactive 2.1.267 sessions) | The session cost tracker (the data behind `/cost`), persisted once at exit: `sessionId`, `totalCostUSD`, `totalAPIDuration`, `totalAPIDurationWithoutRetries`, `totalToolDuration`, `totalDuration` (all durations in ms), `totalLinesAdded`, `totalLinesRemoved`, `startTime` (epoch ms), `modelUsage{<model>: {inputTokens, outputTokens, thinkingTokens, cacheReadInputTokens, cacheCreationInputTokens, webSearchRequests, costUSD}}`, `hasUnknownModelCost`. No `uuid`, `parentUuid`, or `timestamp` |

Findings from the three observed sessions (2026-09-20, all `entrypoint: "cli"`):

- Written exactly once, as the **final record**, immediately after `last-prompt`, so it appears only when the process exits cleanly. Headless runs do not write it: nine `sdk-cli` sessions on the same version (probe transcripts and a scripted `-p` check) end with `last-prompt`, `atis-latch`, or `mode` and never carry it. The two interactive 2.1.236 sessions in the corpus lack it, so it is new in the 2.1.236–2.1.267 window.
- `startTime` is process start, about 15 s before the first conversation record's `timestamp` in every sample; `totalDuration` is wall time from then to exit.
- `modelUsage` is a **superset of the transcript**, which is why it is preserved rather than reconstructed: a helper model (`claude-haiku-4-5-20251001`) appears in every sample and never as an assistant record (title generation and similar calls), `thinkingTokens` has no counterpart in `message.usage`, and even the main model's counts exceed the sum of the transcript's final per-message usage snapshots (output within 0.5%, cache reads 10–20% higher, uncached input roughly 2×; none of the samples had subagents, so the gap is requests that never land in the transcript at all). The adapter therefore treats it as telemetry: `Totals` stay derived from message usage, and adding the two together double-counts.
- Unverified: whether a resumed session's record is cumulative across processes or per-process, whether subagent usage is included, and what an abnormal exit leaves behind (no sample; presumably nothing).
