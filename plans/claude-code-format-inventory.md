...
- Still unverified: what an interrupt exit (Ctrl-C) writes, and where the tracker lives between processes (the resumed process reproduced the first process's numbers, so it is restored from somewhere; whether that is the transcript's own `cost-state` record or separate state is unknown).
